<template>
  <div class="buyUser">
    ...
    <div class="userPhoneAndName">
      <div class="user">
        <div class="uploadPng">
          <Upload action="http://8.140.175.119:8087/NewAddUserHeadPNG" :show-upload-list="false" :data={u_phone:phone} :on-success="getFile">
...
export default {
  ...
  data() {
    return {
      phone: "1",
      userImg: "http://localhost:8080/static/img/1606786397606xiaomao.jpg",
      userItemsList: [
  ...
  methods: {
    ...
    submitPassword() {
      ...
      //发送密码的数据和用户名的数据让用户登录
      this.$axios.post('http://8.140.175.119:8087/submitPassword', this.$qs.stringify({
        ...
        if (res.data == true) {
          ...
          if (this.newPassword == this.confirmNewPwaaword && this.nowPassword != this.newPassword) {
            //用户步骤正确 开始修改密码逻辑  
            this.$axios.post('http://8.140.175.119:8087/submitNewPassword', this.$qs.stringify({
              phone: JSON.parse(localStorage.getItem('user')).u_phone,
    ...
    nameSubmit() {
      //实现修改名称方法  手机号+新名称
      this.$axios.post('http://8.140.175.119:8087/newUserName', this.$qs.stringify({
